...
class images(commands.Cog):
	...
	async def tweet(self, ctx: Interaction, *, text: str):
		...
		embed = nextcord.Embed(title = f"{ctx.user.name} Just Tweeted",color = 0xc6c6f5)		
	...
	async def phcomment(self, ctx: Interaction, *, text):
		...
		embed = nextcord.Embed(title=f"{ctx.user.name} just Commented On P*** Hub", color=0xc6c6f5)
		embed.set_image(url = url)
